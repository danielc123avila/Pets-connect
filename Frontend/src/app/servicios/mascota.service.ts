...
export class MascotaService {
  ...
  getMascotaPorId(id: string): Observable<Mascota> {
    return this.http
      .get<{ success: boolean; data: Mascota }>(
        `${this.urlBase}/mascotas/${id}`
      )
      .pipe(
        tap((response: { success: boolean; data: Mascota }) =>
  ...
  crearMascota(formData: FormData) {
    return this.http
      .post<{ success: boolean; data: Mascota }>(
        `${this.urlBase}/mascotas`,
        formData
      )
      .pipe(
  ...
  actualizarMascota(
    ...
    return this.http
      .put<{ success: boolean; data: Mascota }>(
        `${this.urlBase}/mascotas/${id}`,
        mascota
      )
      .pipe(
        tap((response) =>
          console.log('Respuesta de API (actualizar):', response)
        ),
        map((response) => response.data)
      );
  }
