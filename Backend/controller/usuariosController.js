...
usuariosController.login = async function (request, response) {
    ...
    if (!email || !password) {
        return response.status(400).json({ state: false, mensaje: "Email y contraseña son obligatorios" });
    }

    try {
        const user = await usuariosModel.obtenerUsuarioPorEmail({ email });
        if (!user) {
            return response.status(404).json({ state: false, mensaje: "Email no registrado" });
        }
        ...
        if (!passwordMatch) {
            ...
            return response.status(401).json({ state: false, mensaje: "Contraseña incorrecta" });
        }
...
// Función para el perfil del usuario (protegida por JWT)
usuariosController.perfil = async function (request, response) {
    ...
    if (!token) {
        return response.status(401).json({ state: false, mensaje: "Token no proporcionado" });
    }
    ...
    try {
        ...
        if (!tokenVerificado) {
            return response.status(401).json({ state: false, mensaje: "Token inválido o expirado" });
        }

        // Si el token es válido, continuar con la lógica del perfil
        const userId = tokenVerificado.userId;

        // Obtener la información del usuario desde la base de datos
        const user = await usuariosModel.obtenerUsuarioPorId(userId);
        if (!user) {
            return response.status(404).json({ state: false, mensaje: "Usuario no encontrado" });
        }

        // Responder con la información del perfil del usuario
        response.json({ state: true, usuario: user });
    } catch (error) {
